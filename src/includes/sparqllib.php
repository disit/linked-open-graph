...
class sparql_connection
{
	...
	{
		$url = $this->endpoint . (strpos($this->endpoint, '?') === false ? '?' : '') . "query=" . urlencode($sparql) . ($timeout === null ? "" : "&timeout=$timeout");
		if ($this->params !== null) {
			$url .= "&" . $this->params;
		}
		if ($this->debug) {
		...
		$this->error = null;
		$ch = curl_init($url);
		//fine modifiche zzzzz
		#curl_setopt($ch, CURLOPT_HEADER, 1);
		if ($timeout !== null) {
			curl_setopt($ch, CURLOPT_TIMEOUT, (($timeout / 1000) + 10));
		}
